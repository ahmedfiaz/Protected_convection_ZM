...
!
   ...
   logical  :: zmconv_procon_closure = .FALSE.  
   logical  :: zmconv_procon_cldprp = .FALSE.  
   ...
   !FA
   logical :: procon_closure, procon_cldprp
...
subroutine zmconv_readnl(nlfile)
   ...
   zmconv_procon_closure, zmconv_procon_cldprp
   !-----------------------------------------------------------------------------
   ...
   if (masterproc) then
      ...
      ke = zmconv_ke
      procon_closure = zmconv_procon_closure
      procon_cldprp = zmconv_procon_cldprp
   ...
   call mpibcast(procon_closure,                1, mpilog,  0, mpicom)
   call mpibcast(procon_cldprp,                1, mpilog,  0, mpicom)
...
!FA subroutine that tells if current location is within the protected lattice
subroutine check_inbnds(lchnk,ncol,isinbnds)
   ...
   real(r8), dimension(100):: lat_n,lat_s,lon_e,lon_w
   ...
   logical, intent(inout) :: isinbnds(pcols) ! Logical array that determines if currently within protected
   						   ! region or not
   integer, intent(in) :: lchnk                   ! chunk identifier
   integer, intent(in) :: ncol                    ! number of atmospheric columns

   
    !! Coordinates of the protected array
    
	lat_n=(/15.0, 15.0, 22.5, 37.5, 27.5, 27.5, 26.5, 29.5, 26.5, 35.0, 20.0, 12.5, 5.0, -5.0, -15.0, -25.0, -25.0, -15.0, -5.0, 5.0, -25.0, -15.0, -5.0, 5.0, -15.0, -25.0, -15.0, -7.5, 2.5, 12.5, 22.5, 32.5, -25.0, -15.0, -7.5, 2.5, 12.5, 22.5, 12.5, 2.5, -7.5, -15.0, -25.0, -15.0, -7.5, 2.5, 12.5, 22.5, 18.5, 27.5, 32.5, 17.5, 25.0, 5.0, 5.0, -5.0, -5.0, -15.0, -25.0, 17.5, 25.0, 5.0, -5.0, -15.0, -25.0, -15.0, -25.0, 7.5, 1.5, -5.0, -25.0, 7.5, 17.5, 27.5, 17.5, 7.5, -2.5, -12.5, -22.5, -22.5, -12.5, -2.5, 7.5, 17.5, 27.5, 27.5, 27.5, 17.5, 7.5, -2.5, -12.5, -22.5, 27.5, 17.5, 7.5, -2.5, -12.5, -22.5,-27.5,5.0/)
	lat_s=(/12.5, 12.5, 20.0, 35.0, 25.0, 25.0, 24.0, 27.0, 24.0, 32.5, 17.5, 10.0, 2.5, -7.5, -17.5, -27.5, -27.5, -17.5, -7.5, 2.5, -27.5, -17.5, -7.5, 2.5, -17.5, -27.5, -17.5, -10.0, 0.0, 10.0, 20.0, 30.0, -27.5, -17.5, -10.0, 0.0, 10.0, 20.0, 10.0, 0.0, -10.0, -17.5, -27.5, -17.5, -10.0, 0.0, 10.0, 20.0, 16.0, 25.0, 30.0, 15.0, 22.5, 2.5, 2.5, -7.5, -7.5, -17.5, -27.5, 15.0, 22.5, 2.5, -7.5, -17.5, -27.5, -17.5, -27.5, 5.0, -1.0, -7.5, -27.5, 5.0, 15.0, 25.0, 15.0, 5.0, -5.0, -15.0, -25.0, -25.0, -15.0, -5.0, 5.0, 15.0, 25.0, 25.0, 25.0, 15.0, 5.0, -5.0, -15.0, -25.0, 25.0, 15.0, 5.0, -5.0, -15.0, -25.0,-30.0,2.5/)
	lon_e=(/310.0,285.0, 300.0, 290.0, 290.0, 282.5, 272.5, 257.5, 245.0, 237.5, 237.5, 252.5, 237.5, 245.0, 237.5, 245.0, 260.0, 270.0, 260.0, 270.0, 277.5, 290.0, 300.0, 290.0, 310.0, 325.0, 320.0, 325.0, 320.0, 325.0, 320.0, 325.0, 335.0, 342.5, 335.0, 342.5, 335.0, 342.5, 347.5, 357.5, 7.5, 357.5, 7.5, 20.0, 30.0, 20.0, 30.0, 20.0, 41.5, 52.5, 42.5, 70.0, 77.5, 77.5, 57.5, 50.0, 70.0, 77.5, 70.0, 90.0, 97.5, 97.5, 90.0, 97.5, 90.0, 117.5, 110.0, 110.0, 114.5, 132.5, 132.5, 142.5, 142.5, 142.5, 152.5, 162.5, 152.5, 162.5, 152.5, 172.5, 180.0, 172.5, 180.0, 172.5, 180.0, 162.5, 192.5, 202.5, 192.5, 202.5, 192.5, 202.5, 212.5, 222.5, 212.5, 222.5, 212.5, 222.5,47.5,7.5/)
	lon_w=(/307.5,282.5, 297.5, 287.5, 287.5, 280.0, 270.0, 255.0, 242.5, 235.0, 235.0, 250.0, 235.0, 242.5, 235.0, 242.5, 257.5, 267.5, 257.5, 267.5, 275.0, 287.5, 297.5, 287.5, 307.5, 322.5, 317.5, 322.5, 317.5, 322.5, 317.5, 322.5, 332.5, 340.0, 332.5, 340.0, 332.5, 340.0, 345.0, 355.0, 5.0, 355.0, 5.0, 17.5, 27.5, 17.5, 27.5, 17.5, 39.0, 50.0, 40.0, 67.5, 75.0, 75.0, 55.0, 47.5, 67.5, 75.0, 67.5, 87.5, 95.0, 95.0, 87.5, 95.0, 87.5, 115.0, 107.5, 107.5, 112.0, 130.0, 130.0, 140.0, 140.0, 140.0, 150.0, 160.0, 150.0, 160.0, 150.0, 170.0, 177.5, 170.0, 177.5, 170.0, 177.5, 160.0, 190.0, 200.0, 190.0, 200.0, 190.0, 200.0, 210.0, 220.0, 210.0, 220.0, 210.0, 220.0,45.0,5.0/)
...
subroutine zm_convr(lchnk   ,ncol    , &
                    ...
                    heat    ,pap     ,paph    ,dpp     , &
                    delt    ,mcon    ,cme     ,cape    , cape_undilute, &
...
!
   ...
   !FA output unidlute CAPE
   real(r8), intent(out) :: cape_undilute(pcols)        ! w  convective available potential energy.
...
! FA
   real(r8), intent(out) :: eps(pcols,pver)
...
!! Check if CAPE_UNDILUTE = CAPE_PROTECTED
! do i=1,ncol
!   	 
! 	if (isinbnds(i).eq..TRUE.) then
!       if (cape(i) .eq. cape_undilute(i)) then
! 	     write(iulog,*) '*** PROTECTED CAPE EQUALS UNDILUTE CAPE ****'      
! 
! 	  else
! 	     write(iulog,*) '*** PROTECTED CAPE DOES NOT EQUAL UNDILUTE CAPE ****'      
! 		 call endrun('**** PROTECTED CAPE DOES NOT EQUAL UNDILUTE CAPE ****')	   
!       end if
!    endif
!    
! end do
...
!          tdifr = 0._r8
    ...
    !FA
!          if (tg(i,k) > 0._r8 .or. tg(i,k-1) > 0._r8) &
!             tdifr = abs((tg(i,k)-tg(i,k-1))/max(tg(i,k-1),tg(i,k)))
...
!             that(i,k) = log(tg(i,k-1)/tg(i,k))*tg(i,k-1)*tg(i,k)/(tg(i,k-1)-tg(i,k))
!          else
!             that(i,k) = 0.5_r8* (tg(i,k)+tg(i,k-1))
!          end if
...
if (procon_closure) then 
	call check_inbnds(lchnk,ncol,isinbnds)
	do i = 1, ncol
		if (isinbnds(i)) ke_fac(i)=0.0 
	end do
endif
...
! FA: The undilute CAPE is the same as protected CAPE.
...
!FA: parcel properties are all local variables   
   real(r8) :: tp(pcols,pver)       ! parcel temperature
   real(r8) :: qstp(pcols,pver)     ! saturation mixing ratio of parcel (only above lcl, just q below).
   real(r8) :: tl(pcols)            ! parcel temperature at lcl

   real(r8), intent(out) :: cape(pcols)          ! "undilute" convective aval. pot. energy.
   integer lcl(pcols)        !
   integer lel(pcols)        !
   integer lon(pcols)        ! level of onset of deep convection
   integer mx(pcols)         ! level of max moist static energy
...
! 	  if (isinbnds(i)) then
!     	 hmn(i,k) = cp*t(i,k) + grav*z(i,k) + rl*qst(i,k)         
! 	  else
...
! 		if ((procon_cldprp).and.(isinbnds(i))) then
!         
!         	 if (k >= j0(i) .and. k <= jb(i) .and. hsat(i,k) <= hmin(i)) then
!             	hmin(i) = hsat(i,k)
! 			 end if
!         else
...
! This perturbation has no real change
         	if ((procon_cldprp).and.(isinbnds(i))) then
				expnum(i) = hsat(i,mx(i)) - (hsat(i,k-1)*(zf(i,k)-z(i,k)) + &
...
!
   ...
   do k = pver,msg + 1,-1
      ...
      !FA: set the same entrainment rate up until the theoretical cloud-top
      
      if ((procon_cldprp) .and. isinbnds(i)) then
         if (k < j0(i) .and. k >= jt(i)) eps(i,k) = f(i,j0(i))
      else
         ...
         if (k <= jb(i)-1 .and. k >= lel(i) .and. eps0(i) > 0._r8) then
			...
			if ((procon_cldprp).and.(isinbnds(i))) then
               hu(i,k) = mu(i,k+1)/mu(i,k)*hu(i,k+1) + &
                         dz(i,k)/mu(i,k)* (eu(i,k)*hsat(i,k)- du(i,k)*hsat(i,k))
			else
               hu(i,k) = mu(i,k+1)/mu(i,k)*hu(i,k+1) + &
...
!FA: If updraft MSE is greater than env. MSE, the mass-fluxes are being set to zero
! We want to NOT enforce this condition over protected regions
       if ((procon_cldprp).and.(isinbnds(i))) then  
...
!
      ...
      !FA: why are downdrafts not saturated?
      hd(i,jd(i)) = hmn(i,jd(i)-1)
	  if ((procon_cldprp).and.(isinbnds(i))) then
	      hd(i,jd(i)) = hsat(i,jd(i)-1)
	  endif
   ...
   do k = msg + 1,pver
      do i = 1,il2g
         if ((k >= jt(i) .and. k <= pver) .and. eps0(i) > 0._r8) then
            ...
            !FA make downdrafts entrain saturated air
			if ((procon_cldprp).and.(isinbnds(i))) then
            	hd(i,k) = (md(i,k-1)*hd(i,k-1) - dz(i,k-1)*ed(i,k-1)*hsat(i,k-1))/mdt
			else
...
! 			endif
         ...
         if (( .not. done(i) .and. k > jt(i) .and. k < jb(i)) .and. eps0(i) > 0._r8) then
            !FA: Computing s,q for updrafts
            su(i,k) = mu(i,k+1)/mu(i,k)*su(i,k+1) + &
                      dz(i,k)/mu(i,k)* (eu(i,k)-du(i,k))*s(i,k)
            
            if ((procon_cldprp).and.(isinbnds(i))) then
            qu(i,k) = mu(i,k+1)/mu(i,k)*qu(i,k+1) + dz(i,k)/mu(i,k)* (eu(i,k)*qst(i,k)- &
                            du(i,k)*qst(i,k))
			else
...
!
   do k = msg + 2,pver
      do i = 1,il2g
         if (k >= jd(i) .and. k < jb(i) .and. eps0(i) > 0._r8) then
            qd(i,k+1) = qds(i,k+1)
!FA: evaporation determined by saturated env. air
			if ((procon_cldprp).and.(isinbnds(i))) then
				evp(i,k) = -ed(i,k)*qst(i,k) + (md(i,k)*qd(i,k)-md(i,k+1)*qd(i,k+1))/dz(i,k)
			else
			...
			if ((procon_cldprp).and.(isinbnds(i))) then
	            totevp(i) = totevp(i) - dz(i,k)*ed(i,k)*qst(i,k)
			else
   ...
   if (.false.) then
      do i = 1,il2g
         ...
         if (eps0(i) > 0._r8) then
			...
			!FA: evaporation determined by saturated env. air
			if ((procon_cldprp).and.(isinbnds(i))) then
				evp(i,k) = -ed(i,k)*qst(i,k) + (md(i,k)*qd(i,k))/dz(i,k)
				totevp(i) = totevp(i) - dz(i,k)*ed(i,k)*qst(i,k)
			else
...
! 			if (isinbnds(i)) then
!             dqmdt(i,k) = (mu(i,k+1)* (qu(i,k+1)-qshat(i,k+1)+cp/rl* (su(i,k+1)-s(i,k)))- &
!                           mu(i,k)* (qu(i,k)-qshat(i,k)+cp/rl*(su(i,k)-s(i,k)))+md(i,k+1)* &
!                          (qd(i,k+1)-qshat(i,k+1)+cp/rl*(sd(i,k+1)-s(i,k)))-md(i,k)* &
!                          (qd(i,k)-qshat(i,k)+cp/rl*(sd(i,k)-s(i,k))))/dp(i,k) + &
...
!    call parcel_dilute(lchnk, ncol, msg, mx, p, t, q, tpert, tp, tpv, qstp, pl, tl, lcl)
!FA
   call parcel_dilute(lchnk, ncol, msg, mx, p, t, q, tpert, tp, tpv, qstp, pl, tl, lcl, isinbnds)
...
! FA
subroutine parcel_dilute (lchnk, ncol, msg, klaunch, p, t, q, tpert, tp, tpv, qstp, pl, tl, lcl, isinbnds)
...
! FA:
logical, intent(in), dimension (pcols):: isinbnds
...
!! Enforce this for the protected regions only
 ...
 if (procon_closure) then
        ...
        if (isinbnds(i).eq..FALSE.) then   ! If outside the protected region
        ...
        elseif (isinbnds(i).eq..TRUE.) then

              call qmmr_hPa(tenv,penv,es_env,qs_env)  ! Get the saturated mixing ratio
              senv_sat  = entropy(tenv,penv,qs_env)  ! Saturated entropy of environment 

              sp(i)  = sp(i)  - dmpdp*dp*senv_sat 
              qtp(i) = qtp(i) - dmpdp*dp*qs_env ! Same as undilute plume 
              mp(i)  = mp(i)  - dmpdp*dp
        endif
...
subroutine parcel_undilute (lchnk, ncol, msg, klaunch, p, t, q, tpert, tp, tpv, qstp, pl, tl, lcl)
!subroutine parcel_dilute (lchnk, ncol, msg, klaunch, p, t, q, tpert, tp, tpv, qstp, pl, tl, lcl)
...
!!01/11/19 -FA: Computing "protected CAPE" for all regions.
!
!======================================================================
!
! Set some values that may be changed frequently.
...
!! FA - Obtain the protected CAPE in all regions
...
return
end subroutine parcel_undilute
...
! 
     ...
     real(r8) :: qv,Ts,dTs,fs1,fs2,est,fs3
     real(r8) :: pref,eref,L,e, fac
...
converge: do i=0, LOOPMAX
   ...
   if (i .eq. LOOPMAX - 1) then
          		...
          		! FA: diagnosing the non-convergence of Newton-Raphson scheme
       	  write(iulog,*) '*** NEWTON-RAPHSON METHOD FAILED TO CONVERGE, USING HALLEYS METHOD ****'
       	        
	   Ts = Tfg                  ! Better first guess based on Tprofile from conv.

converge_halley: do j=0, LOOPMAX
		   ...
		   qv = min(qt,qst) 
		   e = qv*p / (eps1 +qv)
		   fs3 = (cpres + qt*cpliq)*log( (Ts+1._r8)/tfreez ) - rgas*log( (p-e)/pref ) + &
				L*qv/(Ts+1._r8) - qv*rh2o*log(qv/qst) - s 
   
   
		   fac=1.0_r8 + (fs1/(fs2-fs1))*(fs3-2*fs1+fs2)/(2*(fs1-fs2))

		  !FA: Implementing Halley's method WHEN Newton's method fails

		   dTs = fs1/(fs2 - fs1)
		   dTs = dTs/fac
		   Ts  = Ts+dTs
		   if (abs(dTs).lt.0.001_r8) exit converge_halley


		  if (j.eq.LOOPMAX-1) then
			...
			' qst(g/kg) = ', 1000._r8*qst,', s(J/kg) = ',s, 'rcall= ',rcall, &
			' Ts = ', Ts, ' dTs = ', dTs, 'e(mb)= ', e,' fs1=', fs1,' fs2=', fs2, &
			' i=',i

!  		  write(iulog,*) 'j=',j,' fs1=',fs1,' fs2=',fs2, ' Ts=',Ts, ' dTs=',dTs,&
!  		  ' qv=',qv,' e=',e,' fs2-fs1=',fs2-fs1
 	     call endrun('**** ZM_CONV IENTROPY: Tmix did not converge ****')	   
   		 endif 
   		end do converge_halley
